...
const vehicleForm = reactive({ plate_number: "", name: "", brand: "", model_name: "", energy_type: "gasoline", seats: 5, status: "available", department: "", custodian: "", purchase_date: "", registration_date: "", purchase_cost: "", company: "", use_scope: "", insurance_started_at: "", current_mileage: 0, insurance_expires_at: "", insurer_name: "", inspection_expires_at: "", vin: "", engine_number: "", asset_card_code: "", asset_number: "", handler_name: "", supervisor_name: "", notes: "" });
  ...
  try {
    ...
    if (props.canManage) {
      [vehicles.value, expenses.value, suppliers.value] = await Promise.all([
        api<Vehicle[]>(vehicleQueryPath()), api<VehicleExpense[]>("/vehicle-expenses/"), api<Supplier[]>("/suppliers/"),
      ]);
    }
...
async function saveVehicle() {
  try {
    const payload = { ...vehicleForm, department: vehicleForm.department ? Number(vehicleForm.department) : null, custodian: vehicleForm.custodian ? Number(vehicleForm.custodian) : null, purchase_date: vehicleForm.purchase_date || null, registration_date: vehicleForm.registration_date || null, purchase_cost: vehicleForm.purchase_cost === "" ? null : vehicleForm.purchase_cost, insurance_started_at: vehicleForm.insurance_started_at || null, insurance_expires_at: vehicleForm.insurance_expires_at || null, inspection_expires_at: vehicleForm.inspection_expires_at || null };
...
function openVehicleNew() {
  ...
  Object.assign(vehicleForm, { plate_number: "", name: "", brand: "", model_name: "", energy_type: "gasoline", seats: 5, status: "available", department: "", custodian: "", purchase_date: "", registration_date: "", purchase_cost: "", company: "", use_scope: "", insurance_started_at: "", current_mileage: 0, insurance_expires_at: "", insurer_name: "", inspection_expires_at: "", vin: "", engine_number: "", asset_card_code: "", asset_number: "", handler_name: "", supervisor_name: "", notes: "" });
...
function openVehicleEdit(item: Vehicle) {
  ...
  Object.assign(vehicleForm, {
    ...
    custodian: item.custodian || "", purchase_date: item.purchase_date || "", registration_date: item.registration_date || "", purchase_cost: item.purchase_cost || "", company: item.company || "", use_scope: item.use_scope || "", current_mileage: item.current_mileage,
    insurance_started_at: item.insurance_started_at || "", insurance_expires_at: item.insurance_expires_at || "", insurer_name: item.insurer_name || "", inspection_expires_at: item.inspection_expires_at || "",
    vin: item.vin || "", engine_number: item.engine_number || "", asset_card_code: item.asset_card_code || "", asset_number: item.asset_number || "", handler_name: item.handler_name || "", supervisor_name: item.supervisor_name || "", notes: item.notes || "",
  });
  showVehicle.value = true;
}
...
<template>
  <div class="page module-page admin-module-page">
    ...
    <div v-if="showVehicle" class="modal-backdrop" @click.self="showVehicle=false"><form class="modal-panel admin-form-modal" @submit.prevent="saveVehicle"><header><div><p class="eyebrow">车辆台账</p><h2>{{ editingVehicle ? '编辑车辆' : '登记车辆' }}</h2></div><button type="button" class="icon-button" @click="showVehicle=false">×</button></header><div class="form-grid"><label><span>车牌号</span><input v-model="vehicleForm.plate_number" required /></label><label><span>车辆名称</span><input v-model="vehicleForm.name" required placeholder="例如：商务车 1 号" /></label><label><span>品牌</span><input v-model="vehicleForm.brand" /></label><label><span>型号</span><input v-model="vehicleForm.model_name" /></label><label><span>能源类型</span><select v-model="vehicleForm.energy_type"><option value="gasoline">汽油</option><option value="diesel">柴油</option><option value="electric">纯电</option><option value="hybrid">混动</option><option value="other">其他</option></select></label><label><span>座位数</span><input v-model.number="vehicleForm.seats" type="number" min="1" /></label><label><span>购置日期</span><input v-model="vehicleForm.purchase_date" type="date" /></label><label><span>初次登记日期</span><input v-model="vehicleForm.registration_date" type="date" /></label><label><span>购置金额</span><input v-model="vehicleForm.purchase_cost" type="number" min="0" step="0.01" /></label><label><span>所属公司</span><input v-model="vehicleForm.company" /></label><label class="wide"><span>使用范围</span><input v-model="vehicleForm.use_scope" /></label><label><span>管理部门</span><select v-model="vehicleForm.department"><option value="">未设置</option><option v-for="x in lookups?.departments||[]" :key="x.id" :value="x.id">{{ x.name }}</option></select></label><label><span>负责人</span><PersonSearchSelect v-model="vehicleForm.custodian" :users="lookups?.users || []" /></label><label><span>当前里程</span><input v-model.number="vehicleForm.current_mileage" type="number" min="0" /></label><label><span>保险开始</span><input v-model="vehicleForm.insurance_started_at" type="date" /></label><label><span>保险到期</span><input v-model="vehicleForm.insurance_expires_at" type="date" /></label><label><span>保险公司</span><input v-model="vehicleForm.insurer_name" /></label><label><span>年检到期</span><input v-model="vehicleForm.inspection_expires_at" type="date" /></label><label><span>车架号</span><input v-model="vehicleForm.vin" /></label><label><span>发动机号</span><input v-model="vehicleForm.engine_number" /></label><label><span>资产卡片编码</span><input v-model="vehicleForm.asset_card_code" /></label><label><span>资产编号</span><input v-model="vehicleForm.asset_number" /></label><label><span>经手人</span><input v-model="vehicleForm.handler_name" /></label><label><span>监管人</span><input v-model="vehicleForm.supervisor_name" /></label><label class="wide"><span>备注</span><textarea v-model="vehicleForm.notes"></textarea></label></div><button class="primary-button full">{{ editingVehicle ? '保存修改' : '保存车辆' }}</button></form></div>
