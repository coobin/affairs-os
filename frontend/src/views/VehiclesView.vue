...
async function createVehicle() {
  try {
    ...
    showVehicle.value = false; await load();
...
async function createExpense() {
  try {
    ...
    showExpense.value = false; await load();
...
<template>
  <div class="page module-page admin-module-page">
    ...
    <section v-else-if="tab === 'vehicles'" class="vehicle-cards"><article v-for="item in vehicles" :key="item.id"><header><span class="vehicle-plate">{{ item.plate_number }}</span><span class="record-status" :data-status="item.status">{{ item.status_label }}</span></header><h2>{{ item.name }}</h2><p>{{ [item.brand,item.model_name].filter(Boolean).join(' ') || '未设置品牌型号' }}</p><dl><div><dt>里程</dt><dd>{{ item.current_mileage.toLocaleString() }} km</dd></div><div><dt>座位</dt><dd>{{ item.seats }} 座</dd></div><div><dt>保险到期</dt><dd>{{ item.insurance_expires_at || '未设置' }}</dd></div><div><dt>年检到期</dt><dd>{{ item.inspection_expires_at || '未设置' }}</dd></div></dl><label class="card-status-control"><span>调整车辆状态</span><select :value="item.status" @change="updateVehicleStatus(item,$event)"><option value="available">可用</option><option value="maintenance">维修保养</option><option value="suspended">停用</option><option value="retired">已处置</option></select></label></article><div v-if="!vehicles.length" class="empty-state large">还没有登记车辆。</div></section>
    <section v-else class="ledger-panel"><div class="panel-actions"><button class="primary-button" @click="showExpense=true">登记车辆事项</button></div><table class="admin-table"><thead><tr><th>日期</th><th>车辆</th><th>事项</th><th>服务商</th><th>里程 / 下次到期</th><th>金额</th></tr></thead><tbody><tr v-for="item in expenses" :key="item.id"><td>{{ item.occurred_on }}</td><td><strong>{{ item.vehicle_label }}</strong></td><td>{{ item.expense_type_label }}<small>{{ item.notes }}</small></td><td>{{ item.supplier_name || '未设置' }}</td><td>{{ item.odometer ? `${item.odometer.toLocaleString()} km` : '—' }}<small>{{ item.next_due_on || (item.next_due_mileage ? `${item.next_due_mileage} km` : '无下次提醒') }}</small></td><td><strong>{{ money(item.amount) }}</strong></td></tr></tbody></table></section>
