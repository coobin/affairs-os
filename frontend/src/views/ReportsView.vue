    ...
    <AppModal v-if="detail" :open="true" :wide="true" :title="detail.title" :description="`共 ${detail.count} 件资产`" @close="closeDetail">
      ...
      <template v-else>
        <div v-if="canBatch && detail.results.length" class="batch-complete-bar">
          ...
          <select v-if="detail.kind === 'department'" v-model="departmentId"><option value="">批量分配保管部门</option><option v-for="item in lookups?.departments || []" :key="item.id" :value="item.id">{{ item.name }}</option></select>
