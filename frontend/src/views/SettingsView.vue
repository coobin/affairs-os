...
async function create() {
  ...
  const payload: Record<string, unknown> = { name: form.name, is_active: true };
  if (tab.value !== "departments") payload.code = form.code;
...
<template>
  <div class="page module-page">
    ...
    <section v-else class="settings-layout">
      ...
      <form v-if="tab !== 'modules'" class="settings-editor" @submit.prevent="create"><p class="eyebrow">新增{{ labels[tab] }}</p><h2>建立基础资料</h2><label><span>名称</span><input v-model="form.name" required /></label><label v-if="tab !== 'departments'"><span>编码</span><input v-model="form.code" required placeholder="使用简短英文或数字" /></label><p v-else class="settings-note">部门编码由系统统一生成，格式为 DEP-001。</p><label v-if="tab === 'categories'"><span>资产分类</span><select v-model="form.class_type"><option value="IT">IT资产</option><option value="ADMIN">行政资产</option></select></label><label v-if="tab === 'locations'"><span>地点类型</span><select v-model="form.kind"><option value="office">办公室</option><option value="warehouse">库房</option><option value="repair">维修点</option><option value="other">其他</option></select></label><label v-if="tab === 'locations'"><span>地址</span><input v-model="form.address" /></label><label v-if="tab === 'categories'"><span>说明</span><input v-model="form.description" /></label><p v-if="error" class="form-error">{{ error }}</p><button class="primary-button full">保存{{ labels[tab] }}</button></form>
    </section>
